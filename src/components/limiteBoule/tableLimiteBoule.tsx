...
import { ILBoule } from "../../types/LimiteBoule";
import { deleteLimiteBoule } from "./actions";
...
export default function TableLimiteBoule() {
  const [filterValue, setFilterValue] = React.useState<string>("");
  const [selectedKeys, setSelectedKeys] = React.useState<any>(new Set([]));
  const [visibleColumns, setVisibleColumns] = React.useState<any>(
    new Set(INITIAL_VISIBLE_COLUMNS)
  );
  const [limites, setLimites] = React.useState<ILBoule[]>([]);
  ...
  React.useEffect(() => {
    const AllLimites = async () => {
      try {
        onSnapshot(collection(db, "limiteboule"), (querySnapshot) => {
  ...
  const renderCell = React.useCallback((boule: ILBoule, columnKey: Ikey) => {
    const cellValue = boule[columnKey];
      ...
      case "active":
        return (
          <Chip
            ...
            color={boule.active ? "success" : "danger"}
            size="sm"
            variant="flat"
          >
            {boule.active ? "Yes" : "No"}
        ...
        <div className="flex justify-between gap-3 items-end px-4 pb-1 pt-4">
          ...
          <div>
            <Button
              ...
              color="primary"
              onClick={() => console.log("rafraichir")}
            >
              Rafraîchir
            </Button>
          </div>
