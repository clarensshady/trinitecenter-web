...
import { ILPA } from "../../types/LimiteJeu";
...
export default function TableLimiteJeuParAgent() {
  const [filterValue, setFilterValue] = React.useState<string>("");
  const [selectedKeys, setSelectedKeys] = React.useState<any>(new Set([]));
  const [visibleColumns, /* setVisibleColumns */] = React.useState<any>(
    new Set(INITIAL_VISIBLE_COLUMNS)
  );
  const [limites, setLimites] = React.useState<ILPA[]>([]);
  ...
  React.useEffect(() => {
    const AllLimites = async () => {
      try {
        onSnapshot(collection(db, "limiteJeuParAgent"), (querySnapshot) => {
  ...
  const renderCell = React.useCallback((jeu: ILPA, columnKey: Ikey) => {
      ...
      case "active":
        return (
          <Chip
            ...
            color={jeu.active ? "success" : "danger"}
            size="sm"
            variant="flat"
          >
            <span className="font-bold">{jeu.active ? "Oui" : "Nom"}</span>
          </Chip>
        );
      default:
        return cellValue;
