...
import { ILBoulePA } from "../../types/LimiteBoule";
...
export default function TableLimiteBouleParAgent() {
  ...
  const [limites, setLimites] = React.useState<ILBoulePA[]>([]);
  ...
  React.useEffect(() => {
    const AllLimites = async () => {
      try {
        onSnapshot(collection(db, "limiteBouleParAgent"), (querySnapshot) => {
  ...
  const renderCell = React.useCallback((boule: ILBoulePA, columnKey: Ikey) => {
      ...
      case "active":
        return (
          <Chip
            ...
            color={boule.active === true ? "success" : "danger"}
            size="sm"
            variant="flat"
          >
            {boule.active === true ? "Yes" : "No"}
          </Chip>
        );
      default:
        return cellValue;
