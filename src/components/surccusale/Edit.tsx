...
export function EditSurcussale() {
  ...
  const configurer = async () => {
    try {
      const surRef = doc(db, "surcussale", `${params.id}`);
      setLoading(true);
      await updateDoc(surRef, {
        ...data,
  ...
  React.useEffect(() => {
    if (isFinished) {
      toast.success("Surcussale modifie avec success", {
        position: "top-right",
        autoClose: 5000,
  ...
  React.useEffect(() => {
    const showSurcusssale = async () => {
      try {
        const surcuRef = doc(db, "surcussale", `${params.id}`);
        const surcuss = await getDoc(surcuRef);
        if (surcuss.exists()) {
          setBlock(surcuss.data().block);
          setData({
            Ville: surcuss.data().Ville,
            NomCentral: surcuss.data().NomCentral,
            Pays: surcuss.data().Pays,
            Superviseur: surcuss.data().Superviseur,
            NumeroTelephone: surcuss.data().NumeroTelephone,
          });
        }
      } catch (error) {
        throw new Error(`${error}`);
