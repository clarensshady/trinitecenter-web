...
import { allBank, allSurcussale, allTirage } from "../../utils/mainActions";
...
export default function FicheVenduTable() {
  ...
  const [dateDebut, setDateDeDebut] = React.useState<CalendarDate>(gdate);
  const [dateDeFin, setDateDeFin] = React.useState<CalendarDate>(gdate);
  ...
  const renderCell = (stats: IFicheVendu, columnKey: Ikey) => {
      ...
      case "actions":
        return (
          <div className="relative flex items-center gap-2">
            <Button
              isIconOnly
              className="px-0 py-0 border-none text-default-400"
              variant="faded"
              aria-label="Take a photo"
              onPress={() => navigate(`pay/${stats.id}`)}
            >
              <FontAwesomeIcon
                color={stats.gagnant ? "#2196f3" : ""}
                icon={faTrophy}
              />
            </Button>

            <Tooltip content="Peyer Fiche">
              <label
                onClick={() => {
                  peyeFiche(`${stats.id}`, stats.peyé);
                }}
              >
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <FontAwesomeIcon
                    color={stats.peyé ? "orange" : ""}
                    icon={faAward}
                  />
                </span>
              </label>
            </Tooltip>
            <Tooltip color="danger" content="Suprimer Fiche">
              <label
                onClick={() => {
                  deleteFiche(stats.id);
                }}
              >
      ...
      <div>
        <div className="flex gap-3 px-2 py-4">
          ...
          <Select
            ...
            value={data.Agent}
            onChange={onChangeSelect}
            className="hidden capitalize md:flex md:max-w-xs"
          >
            {bank.map((agent) => (
