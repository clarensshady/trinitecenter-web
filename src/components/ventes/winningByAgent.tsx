...
type Fiche = {
  ...
  const AllWinningFichePerAgent =  async (dateDebut: string, dateDefin: string, agent?: string, tirage?: string) => {
    ...
    //   const formattedDate = today.toISOString().split('T')[0];
      ...
      } else if (agent == "tout" && tirage == "tout") {
        q = query(
          fichesRef,
          where("timestamp", ">=", Timestamp.fromDate(start)),
          where("timestamp", "<=", Timestamp.fromDate(end)),
      ...
      console.log("next time we got to do it")
  ...
  export { AllWinningFichePerAgent }